...
export default function CreatedSessScreen() {
  ...
  const { createdSess, fetchCreatedSess, isLoading } = useUserActivityStore();

  useEffect(() => {
    if (session?.user?.id) {
      fetchCreatedSess(session.user.id);
    }
  }, [session?.user?.id]);
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => session?.user?.id && fetchCreatedSess(session.user.id)}
            />
          }
          ListEmptyComponent={
            !isLoading ? (
