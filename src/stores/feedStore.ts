    ...
    (set, get) => ({
      ...
      optimisticUnvote: (sesId, optionId) =>
        set((state) => ({
          feed: state.feed.map((s) => {
            if (s.id === sesId) {
              const newSelected = (s.selected_option_ids ?? []).filter(
                (id) => id !== optionId,
              );
              return {
                ...s,
      ...
      updateVoteCount: (sesId, count) =>
        set((state) => ({
          feed: state.feed.map((s) =>
            s.id === sesId ? { ...s, vote_count: count } : s,
          ),
        })),

      prependItem: (item) => set((state) => ({ feed: [item, ...state.feed] })),
          ...
          feed: state.feed.map((item) =>
            item.id === sesId
              ? {
                  ...item,
                  has_voted: true,
                  selected_option_ids: Array.from(
                    new Set([...(item.selected_option_ids ?? []), optionId]),
                  ),
                }
              : item,
          ),
      ...
      markAsUnvoted: (sesId, optionId) =>
        set((state) => ({
          feed: state.feed.map((item) => {
            if (item.id === sesId) {
              const newSelected = (item.selected_option_ids ?? []).filter(
                (id) => id !== optionId,
              );
              return {
                ...item,
